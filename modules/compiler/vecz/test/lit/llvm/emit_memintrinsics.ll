...
; alignment values between LLVM versions.
...
; CHECK: store i64 0, ptr addrspace(1) %[[V35T]], align {{.*}}
; CHECK: %[[V36:[0-9]+]] = getelementptr inbounds i8, ptr addrspace(1) %[[SB_I8AS]], i64 64
; CHECK: store i64 0, ptr addrspace(1) %[[V36]], align {{.*}}
; CHECK: %[[V38:[0-9]+]] = getelementptr inbounds i8, ptr addrspace(1) %[[SB_I8AS]], i64 72
; CHECK: store i64 0, ptr addrspace(1) %[[V38]], align {{.*}}


; Check the loads and stores for the first memcpy
; CHECK:middle:                                           ; preds = %entry
; CHECK: %[[SA_I822:.+]] = load i64, ptr %sa, align {{.*}}
; CHECK: store i64 %[[SA_I822]], ptr addrspace(1) %[[SB_I8AS]], align {{.*}}
; CHECK: %[[SA_I824:.+]] = load i64, ptr %[[V14]], align {{.*}}
; CHECK: store i64 %[[SA_I824]], ptr addrspace(1) %[[V24]], align {{.*}}
; CHECK: %[[SA_I826:.+]] = load i64, ptr %[[V15]], align {{.*}}
; CHECK: store i64 %[[SA_I826]], ptr addrspace(1) %[[V26]], align {{.*}}
; CHECK: %[[SA_I828:.+]] = load i64, ptr %[[V16]], align {{.*}}
; CHECK: store i64 %[[SA_I828]], ptr addrspace(1) %[[V28]], align {{.*}}
; CHECK: %[[SA_I830:.+]] = load i64, ptr %[[V17]], align {{.*}}
; CHECK: store i64 %[[SA_I830]], ptr addrspace(1) %[[V30]], align {{.*}}
; CHECK: %[[SA_I832:.+]] = load i64, ptr %[[V18]], align {{.*}}
; CHECK: store i64 %[[SA_I832]], ptr addrspace(1) %[[V32]], align {{.*}}
; CHECK: %[[SA_I834:.+]] = load i64, ptr %[[V19]], align {{.*}}
; CHECK: store i64 %[[SA_I834]], ptr addrspace(1) %[[V33]], align {{.*}}
; CHECK: %[[SA_I836:.+]] = load i64, ptr %[[V20]], align {{.*}}
; CHECK: store i64 %[[SA_I836]], ptr addrspace(1) %[[V35T]], align {{.*}}
; CHECK: %[[SA_I838:.+]] = load i64, ptr %[[V21]], align {{.*}}
; CHECK: store i64 %[[SA_I838]], ptr addrspace(1) %[[V36]], align {{.*}}
; CHECK: %[[SA_I840:.+]] = load i64, ptr %[[V22]], align {{.*}}
; CHECK: store i64 %[[SA_I840]], ptr addrspace(1) %[[V38]], align {{.*}}

; Check the loads and stores for the second memcpy
; CHECK:end:                                              ; preds = %middle, %entry
; CHECK: %[[SB_I8AS42:.+]] = load i64, ptr addrspace(1) %[[SB_I8AS]], align {{.*}}
; CHECK: store i64 %[[SB_I8AS42]], ptr %result2, align {{.*}}
; CHECK: %[[V42:[0-9]+]] = getelementptr inbounds i8, ptr %result2, i64 8
; CHECK: %[[SB_I8AS44:.+]] = load i64, ptr addrspace(1) %[[V24]], align {{.*}}
; CHECK: store i64 %[[SB_I8AS44]], ptr %[[V42]], align {{.*}}
; CHECK: %[[V43:[0-9]+]] = getelementptr inbounds i8, ptr %result2, i64 16
; CHECK: %[[SB_I8AS46:.+]] = load i64, ptr addrspace(1) %[[V26]], align {{.*}}
; CHECK: store i64 %[[SB_I8AS46]], ptr %[[V43]], align {{.*}}
; CHECK: %[[V44:[0-9]+]] = getelementptr inbounds i8, ptr %result2, i64 24
; CHECK: %[[SB_I8AS48:.+]] = load i64, ptr addrspace(1) %[[V28]], align {{.*}}
; CHECK: store i64 %[[SB_I8AS48]], ptr %[[V44]], align {{.*}}
; CHECK: %[[V45:[0-9]+]] = getelementptr inbounds i8, ptr %result2, i64 32
; CHECK: %[[SB_I8AS50:.+]] = load i64, ptr addrspace(1) %[[V30]], align {{.*}}
; CHECK: store i64 %[[SB_I8AS50]], ptr %[[V45]], align {{.*}}
; CHECK: %[[V46:[0-9]+]] = getelementptr inbounds i8, ptr %result2, i64 40
; CHECK: %[[SB_I8AS52:.+]] = load i64, ptr addrspace(1) %[[V32]], align {{.*}}
; CHECK: store i64 %[[SB_I8AS52]], ptr %[[V46]], align {{.*}}
; CHECK: %[[V47:[0-9]+]] = getelementptr inbounds i8, ptr %result2, i64 48
; CHECK: %[[SB_I8AS54:.+]] = load i64, ptr addrspace(1) %[[V33]], align {{.*}}
; CHECK: store i64 %[[SB_I8AS54]], ptr %[[V47]], align {{.*}}
...
; CHECK: %[[SB_I8AS56:.+]] = load i64, ptr addrspace(1) %[[V35T]], align {{.*}}
; CHECK: store i64 %[[SB_I8AS56]], ptr %[[V48]], align {{.*}}
; CHECK: %[[V49:[0-9]+]] = getelementptr inbounds i8, ptr %result2, i64 64
; CHECK: %[[SB_I8AS58:.+]] = load i64, ptr addrspace(1) %[[V36]], align {{.*}}
; CHECK: store i64 %[[SB_I8AS58]], ptr %[[V49]], align {{.*}}
; CHECK: %[[V50:[0-9]+]] = getelementptr inbounds i8, ptr %result2, i64 72
; CHECK: %[[SB_I8AS60:.+]] = load i64, ptr addrspace(1) %[[V38]], align {{.*}}
; CHECK: store i64 %[[SB_I8AS60]], ptr %[[V50]], align {{.*}}
